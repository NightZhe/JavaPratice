...
<body onload="load()">
	...
	<button type="submit" name="register" onclick="window.location.href='register.jsp'">註冊
		<!-- <a href="register.jsp">註冊</a> -->
	</button>
	<button type="submit" name="forgetPassowrd" onclick="window.location.href='forgetpsd.jsp'">忘記密碼
		...
		$.ajax({
			...
			success: function(response){
					...
					for(i=0; i<response.data.length; i++){					
						$("#table").append("<tr class='tr'> <td> "+response.data[i].id+" </td><td> "+response.data[i].name+" </td> <td> "+response.data[i].password+" </td><td> "+response.data[i].email+" </td> <td> <a href='#' onclick= edit("+i+");> Edit </a>  </td> </td> <td> <a href='#' onclick='delete_("+response.data[i].id+");'> Delete </a>  </td> </tr>");
					}			
			}				
		});
